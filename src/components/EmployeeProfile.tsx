...
import { Mail, Briefcase, Shield, FileText, Calendar, AlertCircle, CheckCircle2, Clock, Plus, Edit2, Trash2, Paperclip, Eye, AlertOctagon, AlertTriangle } from 'lucide-react';
import { motion } from 'motion/react';
import { Employee, Document, DocumentCategory } from '../mockData';
import { getExpiryDetails } from '../utils/expiry';
import { AssignDocumentModal, EditDocumentModal, DeleteConfirmationModal, EditEmployeeModal } from './Modals';
...
export function EmployeeProfile({
  ...
  const employeeDocs = useMemo(() => {
    return documents
      .filter(doc => doc.holder === employee.name)
      .map(doc => ({
        ...doc,
  ...
  const stats = useMemo(() => {
    const expired = employeeDocs.filter(d => d.expiryInfo.status === 'Expired').length;
    const expiring30 = employeeDocs.filter(d => d.expiryInfo.status === 'Expiring < 30d').length;
    const expiring60 = employeeDocs.filter(d => d.expiryInfo.status === 'Expiring < 60d').length;
    const valid = employeeDocs.filter(d => d.expiryInfo.status === 'Valid').length;
    return { expired, expiring30, expiring60, valid };
  }, [employeeDocs]);

  const handleAssignSave = (data: { title: string; type: string; expiryDate: string; hasAttachment: boolean; attachmentName?: string }) => {
    onAssignDocument({
      ...data,
  ...
  const handleEditSave = (data: { expiryDate: string; hasAttachment?: boolean; attachmentName?: string }) => {
    if (activeDoc) {
  ...
  const handleEditEmployeeSave = (data: { title: string; email: string; role: 'Admin' | 'Read-Only' }) => {
    onEditEmployee(employee.email, data);
  };

  const handleDeleteEmployeeConfirm = () => {
    onDeleteEmployee(employee.email);
      ...
      <div className="p-8 border-b border-slate-200 bg-white">
        <div className="flex items-start justify-between">
          <div className="flex items-center">
            ...
            <div className="ml-6">
              <h2 className="text-2xl font-bold text-slate-800 mb-1">{employee.name}</h2>
              <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                <div className="flex items-center">
                  <Briefcase size={14} className="mr-1.5 text-abs-navy" />
                  {employee.title}
                </div>
                <div className="flex items-center">
                  <Mail size={14} className="mr-1.5 text-abs-navy" />
                  {employee.email}
                </div>
                <div className="flex items-center px-2 py-0.5 rounded bg-abs-red border border-abs-red/80 text-xs font-medium text-white">
                  <Shield size={12} className="mr-1 text-abs-steel" />
                  {employee.role}
                </div>
              </div>
            </div>
            ...
            {canManage && (
              <div className="flex items-center space-x-2 mr-2 pr-4 border-r border-slate-200">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-white border border-slate-200 text-slate-400 hover:text-abs-navy hover:border-abs-navy/20 rounded-xl transition-all shadow-sm"
                  title="Edit Employee"
                >
                  <Edit2 size={18} />
                </button>
        ...
        {employeeDocs.length > 0 ? (
              ...
              return (
                <div key={doc.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md transition-all group">
                  ...
                  <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-50">
                    <div>
                      <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-1">Standard Expiry</p>
          ...
          <div className="p-16 text-center border-2 border-dashed border-slate-200 rounded-xl bg-white shadow-sm">
            ...
            <p className="text-slate-400 text-sm max-w-xs mx-auto">
              This employee doesn't have any document records in the system yet.
            </p>
          </div>
        )}
      </div>
